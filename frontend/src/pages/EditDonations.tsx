...
const EditDonation: React.FC = () => {
  ...
  const product = location.state?.product as DonationProduct;
  const fetchDonations = location.state?.fetchDonations;
  const { getToken } = useAuth();

  const [formData, setFormData] = useState<DonationProduct>(product);
  const [imageFile, setImageFile] = useState<File | null>(null);

  useEffect(() => {
    if (!product) {
      navigate("/clothing-bank");
    }
  }, [product, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/clothing_bank/${formData.donation_id}/`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error("Failed to update donation product");
      }

      // Call the callback to re-fetch data
      if (fetchDonations) {
    ...
    <div className="max-w-4xl mx-auto p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="bg-white shadow-lg rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Edit Donation</h2>
          <button
            onClick={() => navigate("/clothing-bank")}
