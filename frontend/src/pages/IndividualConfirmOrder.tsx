...
export function IndividualConfirmOrder() {
  ...
  const product = location.state?.product as SingleProduct;
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      formData.append('total_amount', (product.price * product.quantity + shippingCost).toString());
      formData.append('shipping_address', shippingAddress);
      formData.append('payment_type', paymentMethod);
      formData.append('order_item', product.id);
      formData.append('order_name', product.name);
      formData.append('quantity', product.quantity.toString());

      try {
        const imageFile = await createFileFromPath(product.image);
        formData.append('order_image', imageFile);
      } catch (error) {
        console.error('Error handling image:', error);
        formData.append('order_image', product.image.split('/').pop() || '');
      }
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 pb-4 border-b">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600">NPR {product.price} x {product.quantity}</p>
                <p className="text-gray-700 font-medium">Subtotal: NPR {subtotal}</p>
              </div>
            </div>
