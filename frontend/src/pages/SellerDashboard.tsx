...
const SellerDashboard: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    data.append("price", String(formData.price)); // Convert to string
    data.append("size", formData.size);
    data.append("description", formData.description); // Description field
    data.append("condition", formData.condition);
    data.append("tags", formData.tags); // Matches backend field
    data.append("location", formData.location);

    if (formData.image) {
      data.append("image", formData.image); // Matches backend field
    }

    // Include the authenticated user's ID if required by the backend
    if (user) {
      data.append("clothing_user", user.user_id.toString());
    }
    ...
    try {
      ...
      navigate("/thrift-store"); // Redirect to the thrift store page after successful upload
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="bg-white shadow-lg rounded-lg p-6">
        ...
        <p className="text-gray-600 mb-6">
          As a seller, you can add your products here. Fill out the form below to list your clothes for sale.
        </p>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input
              type="text"
              name="title" // Matches backend field
          ...
          <div>
            ...
            <input
              type="text"
              name="tags" // Matches backend field
              placeholder="Enter tag (e.g., Casual, Denim)"
              value={formData.tags}
              onChange={handleChange}
              className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500"
              required
          ...
          <div>
            ...
            <div className="mt-1 flex flex-col items-center">
              ...
              <input id="fileInput" type="file" name="image" accept="image/*" className="hidden" onChange={handleImageChange} required />
            </div>
          </div>
